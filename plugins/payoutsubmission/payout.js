function AddPlayer(id, name)
{
  ...
  // Placeholder
  var holder = ObjFromID('placeholder');
  if(holder.childNodes[0].nodeName == "TBODY")
    holder = ObjFromID('placeholder').childNodes[0];
  else if(holder.childNodes[1].nodeName == "TBODY")
    holder = ObjFromID('placeholder').childNodes[1];
  ObjFromID('headerrow').style.display = '';
  var rowcount = 0;
  if(holder.childNodes) 
    rowcount = holder.childNodes.length;
